...
export function DeadlineForm({ action, modules, initial, commonFieldsLocked = false }: { action: FormAction; modules: ModuleOption[]; initial?: { title: string; moduleId: string; type: DeadlineType; dueAt: Date; reminderDaysBefore: number | null; weighting: number | null; status: DeadlineStatus; notes: string | null }; commonFieldsLocked?: boolean }) {
  const [state, formAction, pending] = useActionState(action, initialState);
