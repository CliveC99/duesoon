...
import { timetableEventColour, type TimetableEventSummary } from "@/lib/timetable";
...
export function TimetableView({ days, events, todayKey, now }: { days: TimetableDay[]; events: TimetableEventSummary[]; todayKey: string; now: number }) {
  ...
  const nextId = events.filter((event) => event.startAt.getTime() > now).sort((left, right) => left.startAt.getTime() - right.startAt.getTime())[0]?.id;
  ...
  return <><div className="timetable-week-desktop">{days.map((day) => <section className={day.key === todayKey ? "timetable-day timetable-day-today" : "timetable-day"} key={day.key}><header><div><span>{day.label}</span><strong>{formatIrishCalendarDate(day.key)}</strong></div>{day.key === todayKey && <em>Today</em>}</header>{byDay[day.key]?.length ? <div className="timetable-events">{byDay[day.key].map(eventCard)}</div> : <p className="timetable-day-empty">No classes</p>}</section>)}</div><section className="timetable-week-mobile" aria-label="Weekly timetable"><div className="timetable-day-selector" role="tablist" aria-label="Choose a day">{days.map((day) => <button key={day.key} type="button" role="tab" aria-selected={selected === day.key} aria-controls="selected-timetable-day" onClick={() => setSelected(day.key)}><span>{day.shortLabel}</span><strong>{day.dayNumber}</strong>{day.key === todayKey && <small>Today</small>}</button>)}</div><div id="selected-timetable-day" role="tabpanel" className="selected-timetable-day"><header><span>{selected === todayKey ? "Today · " : ""}{formatIrishCalendarDate(selected)}</span><strong>{selectedEvents.length} {selectedEvents.length === 1 ? "class" : "classes"}</strong></header>{selectedEvents.length ? <div className="timetable-events">{selectedEvents.map(eventCard)}</div> : <div className="empty-state"><h2>No classes</h2><p>Nothing is scheduled for this day.</p></div>}</div></section></>;
}
