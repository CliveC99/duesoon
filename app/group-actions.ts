...
async function membership(groupId: string, userId: string) {
  return prisma.groupMember.findUnique({ where: { groupId_userId: { groupId, userId } }, select: { role: true } });
}
...
export async function joinGroup(token: string, _state: GroupActionState, _formData: FormData): Promise<GroupActionState> {
  ...
  try {
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") return { error: "You are already a member of this group." };
    throw error;
  }
  revalidatePath("/groups");
  redirect(`/groups/${group.id}`);
...
export async function updateSharedDeadline(groupId: string, sharedDeadlineId: string, _state: GroupActionState, formData: FormData): Promise<GroupActionState> {
  ...
  const parsed = sharedDeadlineSchema.safeParse(Object.fromEntries(formData));
  if (!parsed.success) return { error: firstError(parsed.error) };
  const shared = await prisma.sharedDeadline.findFirst({ where: { id: sharedDeadlineId, groupId, group: { members: { some: { userId } } } }, select: { createdById: true, group: { select: { ownerId: true } } } });
  ...
  await prisma.$transaction([
    ...
    prisma.deadline.updateMany({ where: { sharedDeadlineId }, data: { title: parsed.data.title, type: parsed.data.type, dueAt: parsed.data.dueAt, weighting: parsed.data.weighting } }),
  ]);
  revalidatePath(`/groups/${groupId}`);
...
export async function importSharedDeadline(groupId: string, sharedDeadlineId: string, _state: GroupActionState, formData: FormData): Promise<GroupActionState> {
  ...
  const [shared, moduleRecord] = await Promise.all([
    prisma.sharedDeadline.findFirst({ where: { id: sharedDeadlineId, groupId, group: { members: { some: { userId } } } } }),
    prisma.module.findFirst({ where: { id: parsed.data.moduleId, userId }, select: { id: true } }),
  ]);
  if (!shared) return { error: "Shared deadline not found." };
  ...
  try {
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") return { error: "This shared deadline is already in your deadlines." };
    throw error;
  }
  revalidatePath(`/groups/${groupId}`);
  revalidatePath("/dashboard");
