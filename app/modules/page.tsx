...
export default async function ModulesPage() {
  ...
  const moduleCard = (module: (typeof unassigned)[number]) => <article className="module-card" key={module.id}><span className="module-colour" style={{ backgroundColor: module.colour }} /><div><p>{module.code || "No code"}</p><h2>{module.name}</h2><span>{module._count.deadlines} {module._count.deadlines === 1 ? "deadline" : "deadlines"}</span></div><div className="card-actions"><Link href={`/modules/${module.id}/edit`}>Edit</Link><DeleteForm action={deleteModule.bind(null, module.id)} label="Delete" /></div></article>;
